...
public class UserController {
    ...
    @Produces (MediaType.APPLICATION_JSON)
    public Response updateUser(UserDTO user,@PathParam("id") String id) throws Exception {
    ...
    public Response createUser(UserDTO user) throws Exception {
        if (userServ.getUserBySsn(user.getSsn())!=null)
            return Response.status(Response.Status.CONFLICT)
                    .entity(
                            new ApiResponse(
                                    409,
            ...
            return Response.status(Response.Status.CREATED)
                    .entity(
                            new ApiResponse(
                                    ...
                                    userServ.getUserBySsn(user.getSsn()),LocalDateTime.now()
                            )
                    )
                    .build();
        return Response.status(Response.Status.CONFLICT)
